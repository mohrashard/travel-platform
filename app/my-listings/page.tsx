...
export default function MyListingsPage() {
    ...
    useEffect(() => {
        async function initializeData() {
            ...
            fetchListings(user.id, false, searchTerm);
        }
    ...
    useEffect(() => {
        ...
        const timer = setTimeout(() => {
            setLoading(true);
            fetchListings(currentUser.id, false, searchTerm);
        }, 500);

        return () => clearTimeout(timer);
    }, [searchTerm, fetchListings, currentUser]);
        ...
        <div className="min-h-screen bg-[#030712] pt-28 pb-12 px-4 md:px-6 lg:px-8 relative overflow-hidden selection:bg-indigo-500/30">
            ...
            <div className="max-w-7xl mx-auto relative z-10">
                <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-12 gap-6">
                    <div>
                        <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-xl bg-cyan-500/10 border border-cyan-500/20 mb-4 backdrop-blur-md">
                            ...
                            <span className="text-xs font-bold text-cyan-300 uppercase tracking-wider">Personal Hub</span>
                        </div>
                        <h1 className="text-4xl md:text-5xl lg:text-6xl font-black text-transparent bg-clip-text bg-gradient-to-r from-white via-neutral-200 to-neutral-400 tracking-tight leading-tight">
                            My <span className="text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 to-indigo-400">Listings</span>
                        </h1>
                        <p className="mt-3 text-lg md:text-xl text-neutral-400 font-light max-w-2xl">
                            Manage the experiences you've created for the world.
                        </p>
                    </div>
                    <Link
                        href="/create"
                ...
                <div className="mb-10 max-w-2xl relative group">
                    <div className="absolute inset-0 bg-gradient-to-r from-cyan-500 to-indigo-500 rounded-2xl blur opacity-20 group-hover:opacity-40 transition-opacity duration-300" />
                ...
                {error && (
                    <div className="mb-10 p-5 bg-red-900/20 border border-red-500/30 text-red-400 rounded-2xl backdrop-blur-md flex items-start gap-4">
                        ...
                        <div>
                            <p className="font-bold text-white mb-1">Error Loading Your Listings</p>
